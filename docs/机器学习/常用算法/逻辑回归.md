...
<img src="http://latex.codecogs.com/svg.latex?J(\theta)=\frac{1}{m}\sum_{i=1}^{m}Cost(h_{\theta}x^{(i)},y^{(i)})">

<img src="http://latex.codecogs.com/svg.latex?Cost(h_{\theta}(x),y)=\begin{cases}-log(h_{\theta}(x))\,if\,y=1\\-log(1-h_{\theta}(x))\,if\,y=0\end{cases},y\in\lbrace0,1\rbrace">
